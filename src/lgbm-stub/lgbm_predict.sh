...
model_table_name=$1
transaction_table_name=$2
param_json=$3

DBNAME=$4

model_csv_base=$(uuidgen).csv
model_csv_remote_path=$REMOTE_DIR/data/csvs/$model_csv_base
model_csv_local_path=$CSVDIR/$model_csv_base
...
(
  psql $DBNAME -c "
    ...
    -- ORDER BY
    --   meshcode
  " -A -F$'\t' | head -n -1 >$model_csv_local_path

  scp $model_csv_local_path $REMOTE_HOST:$model_csv_remote_path
  rm $model_csv_local_path
  ...
  ssh -tt $REMOTE_HOST "$SLURM_CMD $REMOTE_DIR/scripts/lgbm_predict.sh \
    $model_csv_base \
  ...
  ssh $REMOTE_HOST rm $model_csv_remote_path $transaction_csv_remote_path $result_csv_remote_path
) >>$LOGFILE 2>&1
