...
table_name=$(tail -n 1 $INPUTDIR/$model_csv_file | cut -f2)
...
echo "--test_csv=$INPUTDIR/$transaction_csv_file"
...
echo "model_csv_file=$model_csv_file"
...
(
  ...
  python3 $SOURCEDIR/ForOzone/lgbm_predict.py \
    --test_csv $INPUTDIR/$transaction_csv_file \
    --table $table_name \
    --test_date "$test_date" \
    --rank_time $rank_time \
  ...
  mv $SOURCEDIR/output.csv $OUTPUTDIR/$result_csv_file
)
